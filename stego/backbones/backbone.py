...
def get_backbone(cfg):
    ...
    if not hasattr(cfg, "backbone"):
        raise ValueError(
            "Could not find 'backbone' option in the config file. Please check it"
        )
...
class Dinov2ViT(Backbone):
    ...
    def get_backbone_name(self):
        return (
            "DINOv2-"
            + Backbone.vit_name_long_to_short[self.model_type]
            + "-"
            + str(self.patch_size)
        )
...
class DinoViT(Backbone):
    def __init__(self, cfg):
        ...
        if self.model_type == "vit_small" and self.patch_size == 16:
            ...
            raise ValueError(
                "Model type {} unavailable with patch size {}".format(
                    self.model_type, self.patch_size
                )
            )
